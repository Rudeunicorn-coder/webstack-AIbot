/*!
 * ============================================================================
 * WebStackPro Website Chat Widget  v1.0
 * ----------------------------------------------------------------------------
...
(function () {
  ...
  // Brand colors: WebStackPro navy + cyan
  var NAVY = "#0A1F44";
  var CYAN = "#00D4FF";
  ...
  localStorage.setItem(CONFIG_KEY, JSON.stringify({ visitorId: visitorId, name: visitorName }));
  ...
  var css = [
    ...
    "#wsp-fab{display:flex;align-items:center;gap:10px;border:0;cursor:pointer;color:#fff;padding:14px 20px;border-radius:9999px;box-shadow:0 8px 28px rgba(10,31,68,.35);background:" + NAVY + ";font-weight:600}",
    "#wsp-fab:hover{box-shadow:0 8px 34px rgba(0,212,255,.5)}",
    "#wsp-fab-badge{position:absolute;top:-4px;right:-4px;min-width:20px;height:20px;border-radius:9999px;background:" + CYAN + ";color:" + NAVY + ";font-size:11px;font-weight:800;display:flex;align-items:center;justify-content:center;padding:0 5px;display:none}",
    ...
    "#wsp-header{background:" + NAVY + ";color:#fff;padding:14px 16px;display:flex;align-items:center;gap:10px}",
    "#wsp-header h4{font-size:15px;font-weight:700}",
    "#wsp-header p{font-size:11px;opacity:.75}",
    "#wsp-dot{width:8px;height:8px;border-radius:50%;background:" + CYAN + ";box-shadow:0 0 8px " + CYAN + ";animation:wspPulse 1.6s infinite}",
    ...
    ".wsp-customer{align-self:flex-end;background:" + NAVY + ";color:#fff;border-bottom-right-radius:3px}",
    ".wsp-bot{align-self:flex-start;background:#fff;color:#0A1F44;border:1px solid #e2e8f0;border-bottom-left-radius:3px}",
    ".wsp-bot small{display:block;margin-top:3px;font-size:9px;color:#0aa8cc;font-weight:700;text-transform:uppercase}",
    ...
    "#wsp-input:focus{border-color:" + CYAN + ";box-shadow:0 0 0 3px rgba(0,212,255,.2)}",
    "#wsp-send{border:0;background:" + CYAN + ";color:" + NAVY + ";font-weight:700;padding:10px 16px;border-radius:10px;cursor:pointer}",
  ...
  root.innerHTML =
    [
      ...
      '    <div><h4>WebStackPro</h4><p>Powered by WebStackPro AI · replies 24/7</p></div>',
  ...
  function addMessage(role, text) {
    ...
    if (role !== "user") {
      ...
      tag.textContent = "Powered by WebStackPro AI";
      row.appendChild(tag);
    }
  ...
  async function poll() {
    try {
      ...
      if (items.length !== lastMessageCount) {
        ...
        localStorage.setItem(CONFIG_KEY, JSON.stringify({ visitorId: visitorId, name: visitorName, count: lastMessageCount }));
      }
    } catch (err) {
